...
#include "include/ttDMCycleNew.h"
#include "include/ttDMHistsNew.h"
...
void ttDMCycleNew::begin_dataset(Context & ctx){
    // 1. set up selections:
    ...
    // 2. Set up Histograms:
    h_nocuts.reset(new ttDMHistsNew(ctx, "NoCuts"));
    h_btag.reset(new ttDMHistsNew(ctx, "BTag"));
    h_nobtag.reset(new ttDMHistsNew(ctx, "NoBTag"));
    h_topsel.reset(new ttDMHistsNew(ctx, "TopSel"));
    ...
    // this cycle on an output file containing these branches, e.g. one previously produced with ttDMCycleNew ...
    read_example_data = string2bool(ctx.get_setting("readttDMData"));
...
void ttDMCycleNew::process(EventCalc & event, Context & ctx) {
    ...
    if(read_example_data){
        cout << "ttDM data read: int = " << event_intdata_in << "; jet pt = " << event_jetdata_in.pt() << endl;
    }
...
// as we want to run the ttDMCycleNew directly with AnalysisModuleRunner, we have to make sure we are found by the class name.
// This is achieved by calling this macro:
REGISTER_ANALYSIS_MODULE(ttDMCycleNew)
